...
%             Screen('Flip',window);
    ...
    time = clock;
    % RespMat = [BlockAll TrialAll  conditionAll intervalTimesVector  responseVector];
    fileName = ['../../data/GaborDrift/gabor_lineAdjust/1Ori1Dis/' subject_name '-' num2str(time(1)) '-' num2str(time(2)) '-' num2str(time(3)) '-' num2str(time(4)) '-' num2str(time(5)) '.mat'];
    save(fileName,'RespMat','meansubIlluDegree','gaborDistanceFromFixationDegree','gaborMatSingle','intervalTimesVector','cueVerDisDegree','gabor','viewingDistance','trialNumber','blockNumber','lineAngleAll','responseTimeVector');
